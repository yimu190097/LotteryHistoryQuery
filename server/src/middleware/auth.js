...
/**
 * 生成客户端用户 JWT Token（含 jti 用于终端追踪）
 */
function generateUserToken(user) {
  const jti = crypto.randomUUID();
  return {
    token: jwt.sign(
      { phone: user.phone, nickname: user.nickname, isUser: true, jti },
      JWT_SECRET,
      { expiresIn: JWT_EXPIRES }
    ),
    jti
...
function userAuthMiddleware(req, res, next) {
  ...
  try {
    ...
    req.user = { phone: decoded.phone, nickname: decoded.nickname, jti: decoded.jti };
    next();
  } catch (err) {
    return res.status(401).json({ error: 'Token无效或已过期，请重新登录' });
  }
...
module.exports = { generateToken, generateUserToken, authMiddleware, superAdminOnly, userAuthMiddleware, JWT_SECRET, MAX_SESSIONS_PER_USER };
