...
android {
    ...
    // P1-1: 签名密码从项目根目录 signing.properties 读取（该文件已被 .gitignore 排除），
    //       不再硬编码到构建脚本，避免仓库持有任何签名凭据。
    //       仅当 keystore 与密码均存在时才启用 release 签名，否则统一回退到
    //       AGP 内置 debug 签名（保证任何环境都能构建出可安装 APK）。
...
dependencies {
    ...
    // 用 Central 上的 1.1.0：Central 无 1.0.8（该版本仅在 GetStream 私有仓、CI 拉取不稳定），
    // 1.1.0 是 Central 中首个含代码所需 SoftwareAudioDecoderFactory/JavaAudioEncoderFactory/setAudioEncoderFactory 的版本
